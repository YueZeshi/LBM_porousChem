...
class Mesh2D:
    def __init__(self,nx,ny):
        self.nx = nx
        self.ny = ny
        self.S = ti.field(ti.f32,shape=(nx,ny))
        self.L = ti.field(ti.f32,shape=(nx,ny))
        self.d = ti.field(ti.f32,shape=(nx+1,ny+1))
    ...
    def CreateMesh2D(self,shape:str,*params):
        if shape.lower()=="circle":
            self.CreateMesh2DCircle(ti.Vector(params[0]),params[1]) 
    @ti.kernel
    def CreateMesh2DCircle(self, center:TYPE_V2, radius:float): # type: ignore
        for i,j in ti.ndrange(self.nx+1,self.ny+1): # 计算并存储各个格子顶点的距离值
            point = ti.Vector([i-0.5,j-0.5])
            self.d[i,j] = self.circle(point,center,radius)
        for i in ti.grouped(self.S):
            s,l = self.calculate_s_l(self.d[i],self.d[i+ti.Vector([1,0])],self.d[i+ti.Vector([0,1])],self.d[i+ti.Vector([1,1])])
            self.S[i] = s
            self.L[i] = l
