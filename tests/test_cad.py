       ...
       # 3. 转换为规则网格
        ...
        def extract_surface_only():
            ...
            for i in ti.grouped(voxelField):
                if voxelField[i]==0 or i[0]<1 or i[0]>nx or i[1]<1 or i[1]>ny: 
                    surfaceField[i] = 0 
                else:
...
if __name__=="__main__":
    path = "data/stl/sphere.stl"
    r = 0.1
    create_sphere(path,r)

    visualize_stl_basic(path)
    a,b = voxel_stl(path,scale = 1,translate=(0.5,0.5,0),rotate=(0,0,0))
    print(np.shape(a))
    print(np.shape(b))
