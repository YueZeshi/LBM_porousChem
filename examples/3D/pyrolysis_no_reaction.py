...
def main(DX,DT,T_exp,variant="default"):
    ...
    # lb3D.set_species(["wood(S)","tar" ,"char(S)"],
    #                 [True     ,False,True     ])
    # 设置边界条件
    ...
    # # 添加化学反应
    # lb3D.add_reaction("total reaction",[("wood(S)",1)],[("tar",0.4),("char(S)",0.6)],(2500,0,67500,300,0))
    ...
    # ## 热容
    # lb3D.set_specie_capacity("wood(S)",1670)
    # lb3D.set_specie_capacity("char(S)",1000)
    # lb3D.set_specie_capacity("tar",1040)
    # lb3D.set_specie_capacity("N2",1040)
    # ## 热导
    # lb3D.set_specie_conductivity("wood(S)", 0.1256)
    # lb3D.set_specie_conductivity("char(S)", 0.0837)
    ...
    #         for i,j in ti.ndrange(lb3D.nx,lb3D.ny):
    #             total_rad_surface[None] += lb3D.radiation_surface[i,j,0]
    #     def all_rad(lbm:LB3D_PYRO):
    #         cal_all_rad()
    #         return "total_rad_surface", total_rad_surface[None]
    #     lb3D.GetVariableFunc.append(all_rad)
    # setVariables()
    ## 初始化场 
